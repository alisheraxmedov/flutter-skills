---
name: dart
description: Writes or reviews idiomatic Dart 3 code â null safety, var/final/const/late/required, naming, single responsibility, pattern matching, sealed classes. Use when authoring, refactoring, or reviewing any .dart file.
---
...
In a Flutter app, start from the `flutter` skill â it orchestrates these plus the Flutter specialists.
...
- **No abbreviations**: `database`, not `db`; acronyms >2 letters are words â `HttpClient`, `userId` (not `HTTPClient`).
- **Booleans read as predicates**: `isEmpty`, `hasError`, `canRetry` â not `flag`, `status`.
...
- `const` for compile-time constants and constructors â avoid `final` where `const` is legal.
...
- **Extract repeated logic into a named function** â don't copy-paste a block; pull shared logic into a well-named method and call it.

## Critical pitfalls
- Force-unwrap `!`: reserve for proven non-null; prefer `?.`, `??`, `??=`.
- Raw generics: write `List<String>`, never bare `List`; avoid `dynamic` fields.
- `copyWith` with `x ?? this.x` **cannot set a field back to null** â use freezed/sentinel if nulling matters.
...
## Common mistakes
- Magic numbers/strings: `if (role == 'admin')` â name a `const` or `enum` (`Role.admin`).
- Dead code: unused vars/methods/imports â delete them; the analyzer flags them.
- Spaghetti: long function with deep if-else/callback nesting â small functions, early returns, `switch` expressions.
- `late` that may never be set â `late final` set in constructor/`initState`, or make it nullable.
- `dynamic`/raw generics to dodge typing â precise types, bounded generics `<T extends X>`.
...
- Sealed classes, pattern matching, records, class modifiers â full examples: read `reference/patterns.md`.
