...
- **Parallelize independent work** with `Future.wait` (or record `.wait`) â don't chain sequential awaits.
...
- Use `Isolate.run` for CPU work only â not I/O; use async for I/O.
...
## Common mistakes
- Fire-and-forget future â `await` it (try/catch) or mark `unawaited(...)` deliberately; handle `Future.error`.
- `Timer`/`StreamSubscription` never cancelled â cancel in `dispose`/`onClose`; close controllers.
- Empty `catch {}` / silent `catchError` â log and rethrow, or convert to a typed failure.
- Stale state after `await`: the suspension point lets things change â re-check conditions/`mounted` after the gap.

## Gotchas
- **Cancel every `StreamSubscription`** in `dispose`/`onClose`/`tearDown` (and close controllers) â a live subscription pins its callback's whole object graph and leaks.
- **Use `unawaited()` deliberately** â only to mark a genuinely fire-and-forget future; an accidentally dropped future swallows errors and can crash the zone.
- **Prefer `Isolate.run` over `Isolate.spawn`** for one-off CPU work (Flutter: `compute`) â `spawn` is low-level boilerplate; `Isolate.run` is the modern API.
...
## Deep reference
- Futures: await/error handling, `Future.wait` semantics, `unawaited`, debounce/throttle â read `reference/futures.md`.
- Streams: single vs broadcast, `async*`/`yield`, `await for`, cancellation & leak prevention â read `reference/streams.md`.
- Isolates: `Isolate.run`/`compute`, what's sendable, when to use them â read `reference/isolates.md`.
