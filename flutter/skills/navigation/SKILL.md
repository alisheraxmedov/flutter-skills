...
- Baseline (verified 2026-06): `go_router` moves fast and breaks APIs between majors â check the changelog rather than assuming; keep `go_router_builder` on the matching major. Run `flutter pub add` for the exact latest.
...
- **Limit nesting to 2 levels** â deeper trees are hard to reason about.
...
## Passing data
- **Do** use path/query params â deep-linkable, survive cold start.
...
## Gotchas
- **`GoRouter(initialLocation:)` clobbers incoming deep links** on cold start â let the platform route win; gate via `redirect` instead (cross-ref `flutter:deep-linking`).
- **Drive `redirect` off `refreshListenable`, not `ref.watch`/`context.watch`** â `redirect` isn't a widget build; watching there won't re-run it. Pass a `Listenable` (e.g. auth stream) to `refreshListenable`.
- **Typed routes need codegen** â `@TypedGoRoute` requires `go_router_builder` + `build_runner`; forgetting the build step is a known AI mistake.
