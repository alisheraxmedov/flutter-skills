...
```yaml
dependencies: { flutter_riverpod: ^3.3.1, riverpod_annotation: ^3.3.1 }
dev_dependencies: { riverpod_generator: ^3.3.1, build_runner: ^2.4.0 }
```
...
For raw `state = ...` after an await, prefer `keepAlive` for the duration of the operation, or re-fetch through `AsyncValue.guard`. Never store `BuildContext` or `ref` results across an await without re-checking.
