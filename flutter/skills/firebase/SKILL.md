---
name: firebase
description: Integrates Firebase in Flutter â flutterfire configure, stream-based Auth, Firestore with offline persistence, and security rules. Use for firebase_auth, cloud_firestore, authStateChanges, firebase_options.dart, or App Check.
---
...
## Detect first
Match the existing setup â don't re-init a second config:
...
## Setup (essentials)
1. `dart pub global activate flutterfire_cli`, then **`flutterfire configure`** â registers apps and generates `lib/firebase_options.dart`. Never hand-write config.
...
| Use snapshot listeners or explicit `GetOptions(source:)` | Assuming `get()` is fresh â it can return stale cache |
...
| `flutterfire configure` â `firebase_options.dart` | Hand-hardcoding API keys/config |
...
- Writes are **optimistic and offline-queued** â `set/update` resolves locally and syncs later; don't treat a resolved Future as "the server accepted it."

**Security rules reject, they don't filter.** A query that *could* read documents the rules deny **fails entirely** â it does not silently narrow to allowed docs. Write queries that only ever touch permitted documents (e.g. `where('ownerId', isEqualTo: uid)`), and rules that match.

## Gotchas
- **`currentUser` read at startup is null** (known AI mistake) â use `authStateChanges()`; never gate on a synchronous read.
- **`get()` returns cached/stale data offline or after a recent write** (known AI mistake) â use listeners or `GetOptions(source:)`.
- **`allow read, write: if true;`** (top AI mistake) â test-mode rules left in prod expose the whole database. Replace before launch.
- **Rules don't filter queries** (known AI mistake) â a too-broad query throws `permission-denied` instead of returning a subset.
- **`firebase_options.dart` and `google-services.json`/`GoogleService-Info.plist` are config, not secrets** â but keep them out of public VCS; security comes from **rules + App Check**, not from hiding them.
- **Forgot `await Firebase.initializeApp` before use** â `[core/no-app]`. Initialize in `main` before `runApp`.
- **Hot-restart vs cold-start differ** for auth â test session restore on a real cold start, not just hot restart.

## Common mistakes
- Gating UI on `currentUser` at startup â use `authStateChanges()`.
- `get()` everywhere expecting fresh data â snapshot listeners or explicit source.
- Treating an offline write's resolved Future as server confirmation â handle eventual consistency.
- Shipping `if true` rules â enforce auth + ownership and add App Check.
- Broad queries the rules deny â scope the query (`where ownerId == uid`) to match the rules.
