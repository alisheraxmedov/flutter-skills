...
## Essential rules
- **One configured Dio** (base URL, timeouts, headers), built once and injected via DI â never `Dio()` per request.
...
- **retrofit for the client**: declare an abstract `@RestApi()` interface; let it serialize. No `jsonDecode`, no `try/catch` inside the interface â let `DioException` propagate.
- **Map `DioException` â typed `Failure`** in one mapper (see error-handling skill for the `Failure`/`Result` types).
...
| Mapper | `DioException` â domain `Failure` |
| Repository | Catch at boundary, return `Result<T>`, map DTO â domain |

## Cancellation
Pass a `CancelToken` from the caller (cancel on query change / screen dispose). A cancelled request throws `DioException` type `cancel` â map it as a benign failure.

## Common mistakes
- Hardcoded base URLs / API keys in the client â inject via config / `Env` / `--dart-define`; never commit secrets to source.
- `LogInterceptor` dumping auth headers/bodies in release â enable logging only in debug (`kDebugMode`) and redact `Authorization`.
- `Dio()` per request â build one configured Dio once and inject it via DI.
- `try/catch` in the UI for network calls â catch only in the repository boundary, return `Result<T>`.
- See `reference/anti-patterns.md` for full do/avoid.

## Gotchas
- **dio v5 timeouts are `Duration`** â `connectTimeout`/`receiveTimeout`/`sendTimeout` take `Duration`, not `int` ms; the old `int` form is a known AI mistake.
- **Never `badCertificateCallback: (_, __, ___) => true`** to silence TLS errors â that disables cert validation (cross-ref `flutter:security`).
- **No manual `jsonDecode`/`try/catch` in retrofit interfaces** â let the generated client (de)serialize and let `DioException` propagate.
...
- Full `DioException` â `Failure` mapper: read `reference/error-mapping.md`.
